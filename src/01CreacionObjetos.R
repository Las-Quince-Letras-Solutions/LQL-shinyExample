...
df <- data.frame(fecha = rep(1:104, times = 4),
...
df %>% 
  ggplot() +
  geom_line(aes(fecha, ventas, color = region), size = 0.75) +
  geom_smooth(aes(fecha, ventas, color = region), 
...
mod.lm <- lm(ventas ~ .,
             data = select(b, -fecha, -region.Valle.de.México))
